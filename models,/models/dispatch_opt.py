...
def optimize_dispatch(
    ...
    q_csp = np.nan_to_num(q_csp, nan=0.0)
    e_pv  = np.nan_to_num(e_pv,  nan=0.0)
    ...
    if tc != pyo.TerminationCondition.optimal:
        ...
        try:
            log_infeasible_constraints(m, tol=1e-7)
        except Exception:
            pass
...
def calculate_firm_bonus(res, base_price=1.0, bonus_rate=0.2, margin=0.05, min_hours=3, dt=1.0):
    ...
    P_exp = np.array(res["pv_to_grid_MWe"]) + (np.array(res["pb_electric_MWe"]) )
...
def plot_stack_flows(out, start_hour=4104, hours=4248, dt=1.0, save_as=None):
    ...
    plt.title(f"Heat generation from hour {start_hour} to {start_hour+hours}")
...
def plot_stack_flows_elc(out, start_hour=4104, hours=4248, dt=1.0, save_as=None):
    ...
    x = dfw["hour"]
    y1 = dfw["pv_to_grid_MWe"]
    y2 = dfw["pb_electric_MWe"]
    # y3 = dfw["csp_to_pb"]
    # y4 = dfw["spill_csp"]
    
    fig, ax = plt.subplots(figsize=(12,4))
    ax.stackplot(x, y1, y2, 
                 labels=['PV→Grid','PB→Grid'],
                 colors=['tab:cyan','tab:orange'],
                 alpha=0.8)
    
    ax.set_xlabel("Hour since start")
    ax.set_ylabel("Power (MW-e)")
    ax.legend(loc='upper left')
    ax.grid(True)
    # plt.title(f"Stacked flows from hour {start_hour} to {start_hour+hours}")
    plt.title(f"Electricity generation from hour {start_hour} to {start_hour+hours}")
...
def plot_shares_price(out,
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list(
        ...
        ["#fb037f", "#85e834", "#18B4F2", "#b103fb", "#fb6a03"]
    )
    ...
    for autotext in autotexts:
        ...
        autotext.set_color("#282828")  # or choose a contrasting colour
