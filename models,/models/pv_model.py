...
def run_pvsam(weather_path: str = "input_data/tmy_38.730_-3.450_2005_2023.epw",
              module_name: str = "Suntech Power STP330-24/Vem",
    ...
    if "desired_array_size" in system_kwargs:
        p_ac = float(system_kwargs["desired_array_size"])/dc_to_ac_ratio
        n_inv = math.ceil(p_ac/(float(iv.inv_snl_paco)/1000))
        n_mod_string = math.floor(float(iv.inv_snl_vdcmax)/float(g.cec_v_mp_ref))
        p_mod =(g.cec_v_mp_ref*g.cec_i_mp_ref)
        n_strig_parallel = math.ceil(float(system_kwargs["desired_array_size"])*1000/(n_mod_string*p_mod))
        print(f"n_inv = {n_inv}")
        print(f"n_mod_string = {n_mod_string}")
        print(f"p_mod = {p_mod}")
        print(f"n_strig_parallel = {n_strig_parallel}")

    # ---- SYSTEM DESIGN ----
    pv.SystemDesign.inverter_count = n_inv
    pv.SystemDesign.subarray1_modules_per_string = n_mod_string
    pv.SystemDesign.subarray1_nstrings = n_strig_parallel
    ...
    # land_area_m2 = float(pv.HybridCosts.land_area)
