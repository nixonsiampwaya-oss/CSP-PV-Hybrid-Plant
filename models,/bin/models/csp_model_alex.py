...
def run_basic_mspt(
    ...
    # Design optimal Field and Reciever geometry
    mspt.HeliostatField.field_model_type = 0        # Designs the optimal number and alignment of heliostats, also of the reciever

    # System Costs
    mspt.SystemCosts.tower_fixed_cost = 1.5e6*160
    mspt.SystemCosts.tower_exp = 0.0113             # tower_cost = tower_fixed_cost * exp ( tower_exp * f(tower_height, heliostat_height))
    mspt.SystemCosts.rec_ref_cost = 55e6           
    mspt.SystemCosts.rec_ref_area = 1571                    
    mspt.SystemCosts.rec_cost_exp = 0.7             # rec_cost = f(rec_ref_cost, rec_ref_area, rec_cost_exp)    
    mspt.SystemCosts.site_spec_cost = 2             # USD/m2
    mspt.SystemCosts.heliostat_spec_cost = 115      # USD/m2 = Solar aperture Area
    mspt.SystemCosts.plant_spec_cost = 1040         # Power block cost
    mspt.SystemCosts.bop_spec_cost = 680            # BOP USD/kW
    mspt.SystemCosts.tes_spec_cost = 22             # USD/kWh-th
    mspt.SystemCosts.land_spec_cost = 2*4047        # USD/acre
    mspt.SystemCosts.contingency_rate = 7           # Contingency for cost overrun [%]
    mspt.SystemCosts.sales_tax_frac = 80            # Percent of cost to which sales tax applies [%]
    mspt.SystemCosts.csp_pt_cost_epc_percent = 13   # EPC cost percent of direct [%]

    # Financial Parameters
    mspt.FinancialParameters.sales_tax_rate = 5                 # Sales tax rate [%]
    mspt.FinancialParameters.const_per_interest_rate1 = 6.5     # Interest rate, loan 1 [%]
    ...
    reciever_height = float(getattr(mspt.Outputs, "rec_height_calc", float("nan")))
    reciever_diameter = float(getattr(mspt.Outputs, "D_rec_calc", float("nan")))
    N_heliostats = float(getattr(mspt.Outputs, "N_hel_calc", float("nan")))
    ...
    csp_summary = {
        ...
        "Reciever_Diameter_m" : reciever_diameter,
        "Reciever_Height_m" : reciever_height,
